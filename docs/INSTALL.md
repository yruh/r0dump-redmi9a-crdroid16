...
本次 Download-only 最终修复的已签名包位于源码树之外：

```text
crDroidAndroid-16.0-20260815-blossom-v12.11-download-only-final-v2.zip
```

它从已验证基线 OTA 生成，更新 `system` 的 ART/framework/SELinux 和
`system_ext` 的 Manager，保持 boot、recovery、vendor、product、odm、dtbo、
vbmeta 及各分区 transfer list 不变。
